...
// Fonction qui lit et retourne la comparaison de deux points
...
// Renvoie sur un point c est à droite (-1) ou à gauche (1) d'une droite reliant les points a et b
...
// Calcul la distance entre deux points et le pivot sur lequel nous sommes actuellement.
// On peut trouver l'élément le plus à gauche ainsi
...
// Début de l'algorithme de jarvis
  const auto jarvis = [&](std::vector<Point>& points)
  {
    trace.beginBlock("Jarvis");

// On récupère le nombre de points de notre figure
    const int N = points.size();

// Ecriture sur le terminal du nombre de points à traiter
    trace.info() << "Jarvis on " << N << " points." << std::endl;
    std::stack<Point> hull;

// Si N<3, il ne peut pas y avoir de points hors des extrémités, donc on le renvoie tel quel.
    if (N < 3)
      return hull;
    ...
    do
    {
      // Le pivot devient le dernier point de notre hull
      pivot = hull.top();
      Point p = points[0];
      // Boucle for pour trouver le point le plus à droite de notre fonction.
      for (int i = 1; i < N; i++)
      {
