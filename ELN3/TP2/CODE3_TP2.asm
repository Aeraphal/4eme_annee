...
;	On obtient un tableau dont les données sont mal triées et en plus modifiée.
;	Il y a même des pertes dans les données obtenues.
...
;-  Expliquez la nature de votre modification et pourquoi vous la faites.

;	On modifie la pile de place car dans sa précédente position elle écrasait
;	des données ce qui détruit des sauvegardes importantes de l'algorithme principale.
;	Ainsi, on peut qualifier le changement de simple relocalisation de la pile.
;
...
;	Il faut faire attention à bien définir nos espaces de mémoire afin de ne pas écraser
;	des informations que nous souhaitons créer.
...
BCL:    mov 50H,#22H  ; Remplissage de la mémoire de 50H à 57H
		...
		mov sp,#30h		; ON A CHANGE LE PROGRAMME ICI
